...
**Версия 2+ (19 байт, с поддержкой сжатия):**
```
Offset | Размер | Тип      | Описание
-------|--------|----------|------------------
0x00   | 4      | bytes    | Magic number: "RGBA"
0x04   | 1      | uint8    | Версия формата: 2
0x05   | 1      | uint8    | Compression flag: 0 или 1 (gzip)
0x06   | 4      | uint32   | Количество кадров (little-endian)
0x0A   | 4      | uint32   | Максимальная ширина (little-endian)
0x0E   | 4      | uint32   | Максимальная высота (little-endian)
0x12   | 1      | uint8    | Grayscale флаг: 0 или 1
```
...
Начиная с версии 2 формата, поддерживается **gzip сжатие** без потери качества:
- Сжатие применяется ко всему блоку кадров
- Типичное уменьшение размера: **60-80%** для raw RGBA данных
- Включено по умолчанию (`useCompression: true`)
- Можно отключить для совместимости или отладки

### Примеры размеров

Для записи 10 секунд при 10 FPS (100 кадров, 360x640):

- **Без сжатия**: ~92 MB
- **Со сжатием gzip**: ~15-25 MB (уменьшение на 70-85%)
- **С resizeRatio 0.3**: ~8-12 MB (со сжатием)

## Преимущества нового формата

1. ✅ **Без потери качества** - сырые RGBA данные
2. ✅ **Сжатие gzip** - уменьшение размера на 60-80% без потери качества
3. ✅ **Простое декодирование** - любой язык программирования
4. ✅ **Гибкость** - можно применять любую обработку на backend
5. ✅ **Меньше зависимостей** - не нужны библиотеки для GIF
6. ✅ **Метаданные** - timestamp для каждого кадра
7. ✅ **Совместимость** - те же настройки размеров
